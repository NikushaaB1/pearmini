...
-- ═══════════════════════════════════════════════════════════════
-- Social feed — პოსტები, კომენტარები, რეაქციები
-- (იხ. ასევე supabase/feed_posts.sql)
-- ═══════════════════════════════════════════════════════════════
...
create policy "feed_posts_insert_authenticated" on public.feed_posts for insert to authenticated with check (author_uid = auth.uid());
drop policy if exists "feed_posts_update_authenticated" on public.feed_posts;
create policy "feed_posts_update_authenticated" on public.feed_posts for update to authenticated using (true) with check (true);
drop policy if exists "feed_posts_delete_own_or_admin" on public.feed_posts;
create policy "feed_posts_delete_own_or_admin" on public.feed_posts for delete to authenticated using (author_uid = auth.uid() or public.is_admin());

drop policy if exists "feed_comments_select_authenticated" on public.feed_post_comments;
create policy "feed_comments_select_authenticated" on public.feed_post_comments for select to authenticated using (true);
drop policy if exists "feed_comments_insert_authenticated" on public.feed_post_comments;
create policy "feed_comments_insert_authenticated" on public.feed_post_comments for insert to authenticated with check (author_uid = auth.uid());
drop policy if exists "feed_comments_delete_own_or_admin" on public.feed_post_comments;
create policy "feed_comments_delete_own_or_admin" on public.feed_post_comments for delete to authenticated using (author_uid = auth.uid() or public.is_admin());
