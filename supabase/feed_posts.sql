-- ═══════════════════════════════════════════════════════════════
-- PEAR Elite — Social feed (პოსტები + კომენტარები + რეაქციები)
-- გაუშვი Supabase Dashboard → SQL Editor (ერთხელ)
-- ═══════════════════════════════════════════════════════════════
...
comment on table public.feed_posts is 'სოციალური ფიდის პოსტები';
comment on column public.feed_posts.reactions is 'JSON: { "like": ["uid1"], "love": ["uid2"], ... }';
...
create index if not exists feed_posts_model_id_idx on public.feed_posts (model_id) where model_id is not null;
...
comment on table public.feed_post_comments is 'პოსტის კომენტარები';

create index if not exists feed_post_comments_post_id_idx on public.feed_post_comments (post_id);
create index if not exists feed_post_comments_created_at_idx on public.feed_post_comments (created_at);

-- RLS
alter table public.feed_posts enable row level security;
alter table public.feed_post_comments enable row level security;

-- პოსტები: ყველა authenticated ხედავს
...
-- პოსტები: მხოლოდ საკუთარი author_uid-ით
...
-- რეაქციები (reactions jsonb) — ნებისმიერი authenticated
